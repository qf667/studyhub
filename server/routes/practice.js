...
// 错题本
router.get('/wrong-questions', authMiddleware, roleGuard('student'), (req, res) => {
...
// 标记已掌握
router.put('/wrong-questions/:id/master', authMiddleware, roleGuard('student'), (req, res) => {
  run('UPDATE wrong_questions SET is_mastered=1 WHERE id=? AND student_id=?', [req.params.id, req.user.id]);
  res.json({ code: 200, message: '已标记为掌握' });
});

// 按知识点练习
router.post('/start', authMiddleware, roleGuard('student'), (req, res) => {
...
// 提交练习答案
router.post('/submit', authMiddleware, roleGuard('student'), (req, res) => {
  ...
  for (const a of answers) {
    const question = getOne('SELECT * FROM questions WHERE id=?', [a.question_id]);
    if (!question) continue;
    const isCorrect = a.user_answer?.toString().trim().toLowerCase() === question.answer?.toString().trim().toLowerCase();
    if (isCorrect) totalCorrect++;
    else {
      const exists = getOne('SELECT id FROM wrong_questions WHERE student_id=? AND question_id=?', [req.user.id, a.question_id]);
      if (!exists) {
        run('INSERT INTO wrong_questions (student_id, question_id, wrong_answer) VALUES (?, ?, ?)',
