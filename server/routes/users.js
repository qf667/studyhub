...
router.get('/', authMiddleware, roleGuard('admin'), (req, res) => {
  ...
  const countRow = getOne(`SELECT COUNT(*) as total FROM users ${whereClause}`, params);
...
// 分配角色（管理员）
router.put('/:id/role', authMiddleware, roleGuard('admin'), (req, res) => {
  const { id } = req.params;
  const { role } = req.body;
...
// 启用/禁用（管理员）
router.put('/:id/status', authMiddleware, roleGuard('admin'), (req, res) => {
  const { id } = req.params;
  const { status } = req.body;
  run('UPDATE users SET status=?, updated_at=CURRENT_TIMESTAMP WHERE id=?', [status ? 1 : 0, id]);
  res.json({ code: 200, message: status ? '已启用' : '已禁用' });
});

// 批量导入（简化版）
router.post('/batch-import', authMiddleware, roleGuard('admin'), (req, res) => {
...
router.get('/classes', authMiddleware, (req, res) => {
  const classes = getAll("SELECT DISTINCT class_name FROM users WHERE class_name IS NOT NULL AND class_name != ''");
