...
router.get('/', authMiddleware, asyncHandler(async (req, res) => {
  ...
  const countRow = await getOne(`SELECT COUNT(*) as total FROM courses c ${joinClause} ${whereClause}`, params);
...
// 提交审核
router.post('/:id/submit', authMiddleware, roleGuard('teacher'), asyncHandler(async (req, res) => {
  await run('UPDATE courses SET status=? WHERE id=? AND teacher_id=?', ['pending', req.params.id, req.user.id]);
  res.json({ code: 200, message: '已提交审核' });
}));

// 管理员审核课程
router.put('/:id/review', authMiddleware, roleGuard('admin'), asyncHandler(async (req, res) => {
  const { status, reason } = req.body;
  if (!['approved', 'rejected'].includes(status)) {
    return res.json({ code: 400, message: '无效的审核状态' });
  }
  await run('UPDATE courses SET status=? WHERE id=?', [status, req.params.id]);
  res.json({ code: 200, message: status === 'approved' ? '审核通过' : '已拒绝' });
}));

// 分配课程给学生
router.post('/:id/assign', authMiddleware, roleGuard('admin', 'teacher'), asyncHandler(async (req, res) => {
  ...
  if (Array.isArray(student_ids)) {
    ...
    const students = await getAll('SELECT id FROM users WHERE class_name=? AND role=?', [class_name, 'student']);
  ...
  for (const sid of ids) {
    try {
      await run(
        'INSERT INTO course_assignments (course_id, student_id, assigned_by, deadline) VALUES (?, ?, ?, ?) ON CONFLICT (course_id, student_id) DO NOTHING',
...
// 取消分配
router.delete('/:id/assign/:studentId', authMiddleware, roleGuard('admin', 'teacher'), asyncHandler(async (req, res) => {
  await run('DELETE FROM course_assignments WHERE course_id=? AND student_id=?',
    [req.params.id, req.params.studentId]);
  res.json({ code: 200, message: '取消分配成功' });
}));

// 查看课程下的学生列表
router.get('/:id/students', authMiddleware, asyncHandler(async (req, res) => {
...
// 获取学科分类列表
router.get('/meta/subjects', authMiddleware, asyncHandler(async (req, res) => {
  const subjects = await getAll("SELECT DISTINCT subject FROM courses WHERE subject IS NOT NULL AND subject != ''");
