...
router.post('/', authMiddleware, roleGuard('admin', 'teacher'), asyncHandler(async (req, res) => {
  ...
  const homeworkId = await lastId();

  if (Array.isArray(question_ids)) {
    for (let i = 0; i < question_ids.length; i++) {
      const q = await getOne('SELECT score FROM questions WHERE id=?', [question_ids[i]]);
    ...
    await run('UPDATE homework SET total_score=? WHERE id=?', [totalScore, homeworkId]);
  }
...
// 发布作业
router.post('/:id/publish', authMiddleware, roleGuard('admin', 'teacher'), asyncHandler(async (req, res) => {
  await run('UPDATE homework SET status=? WHERE id=?', ['published', req.params.id]);
  res.json({ code: 200, message: '发布成功' });
}));

// 删除作业
router.delete('/:id', authMiddleware, roleGuard('admin', 'teacher'), asyncHandler(async (req, res) => {
  await run('DELETE FROM homework_questions WHERE homework_id=?', [req.params.id]);
  await run('DELETE FROM homework WHERE id=?', [req.params.id]);
  res.json({ code: 200, message: '删除成功' });
}));

// 学生提交作业
router.post('/:id/submit', authMiddleware, roleGuard('student'), asyncHandler(async (req, res) => {
  const { answers } = req.body;
  const homeworkId = req.params.id;
  if (!answers) return res.json({ code: 400, message: '答案不能为空' });

  const homework = await getOne('SELECT * FROM homework WHERE id=?', [homeworkId]);
  if (!homework) return res.json({ code: 404, message: '作业不存在' });
  ...
  const hwQuestions = await getAll(
...
// 查看某作业的提交情况（教师端）
router.get('/:id/submissions', authMiddleware, roleGuard('admin', 'teacher'), asyncHandler(async (req, res) => {
...
// 教师批改
router.put('/submissions/:id/grade', authMiddleware, roleGuard('admin', 'teacher'), asyncHandler(async (req, res) => {
...
// 我的提交记录（学生端）
router.get('/submissions/mine', authMiddleware, roleGuard('student'), asyncHandler(async (req, res) => {
  const submissions = await getAll(
