...
router.get('/', authMiddleware, (req, res) => {
  ...
  const countRow = getOne(`SELECT COUNT(*) as total FROM questions q ${whereClause}`, params);
...
// 批量操作
router.post('/batch', authMiddleware, roleGuard('admin', 'teacher'), (req, res) => {
  const { action, ids, data } = req.body;
  if (!Array.isArray(ids) || ids.length === 0) return res.json({ code: 400, message: '请选择题目' });
  const placeholders = ids.map(() => '?').join(',');
  if (action === 'delete') {
    run(`DELETE FROM questions WHERE id IN (${placeholders})`, ids);
  } else if (action === 'update_difficulty' && data?.difficulty) {
    run(`UPDATE questions SET difficulty=? WHERE id IN (${placeholders})`, [data.difficulty, ...ids]);
  } else if (action === 'update_score' && data?.score) {
    run(`UPDATE questions SET score=? WHERE id IN (${placeholders})`, [data.score, ...ids]);
  }
