  ...
  <el-container style="height: 100vh">
    ...
    <el-aside :width="sidebarWidth" class="app-sidebar" :class="{ collapsed: theme.sidebarCollapsed, mobile: theme.isMobile }">
      <div class="sidebar-header">
        <span v-if="!theme.sidebarCollapsed">StudyHub 教师端</span>
        <span v-else>SH</span>
      </div>
      <el-menu :default-active="route.path" router :collapse="theme.sidebarCollapsed" background-color="transparent" text-color="var(--sidebar-text)" active-text-color="var(--sidebar-active)">
        ...
        <el-menu-item index="/teacher/ai-questions">
        ...
        <div style="display: flex; align-items: center; gap: 12px">
          ...
          <el-dropdown @command="handleCommand">
            <span style="cursor: pointer; display: flex; align-items: center; gap: 8px">
              <el-avatar :size="32">{{ auth.user?.real_name?.charAt(0) || 'T' }}</el-avatar>
...
const currentTitle = computed(() => {
  if (route.path === '/teacher') return '我的课程'
  if (route.path === '/teacher/ai-questions') return 'AI智能出题'
  if (route.path.includes('/teacher/course/')) return '课程详情'
  if (route.path.includes('/grade')) return '作业批改'
  return '教师中心'
})
